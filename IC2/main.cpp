...
#define MAX_MSG_LENGTH      64 /* max length */
...
int main() {
    ...
    if(pid == 0){
        ...
        shared_object = shm_open(SHARED_OBJ_NAME, O_CREAT | O_EXCL | O_RDWR, S_IRWXU);
        if (shared_object < 0) {
	    ...
	    strcpy(shared_message->content, "Message (:");
    ...
    if(pid > 0){
        ...
        cout << "Parent process " << getpid() << " will read message from process " << pid << " finished with status "<< exit_status << endl;

        // Flag S_IRWXU: Read, write, and execute by owner.
